...
raw_train_loader = tokenize_shards(train_shard_indices, config.device_batch_size * 8, config.seq_len)
train_loader = PrefetchDataLoader(raw_train_loader, capacity=4)
val_loader_fn = lambda: tokenize_shards(val_shard_indices, config.device_batch_size * 8, config.seq_len)

# Pre-generate random data on HBM for profiling (excludes data loading time)
if use_random_data:
    rng_data = jax.random.key(0)
    random_x = jax.random.randint(rng_data, (config.device_batch_size * 8, config.seq_len),
                                   0, config.vocab_size, dtype=jnp.int32)
    random_y = jax.random.randint(rng_data, (config.device_batch_size * 8, config.seq_len),
                                   0, config.vocab_size, dtype=jnp.int32)
    # Force to device
